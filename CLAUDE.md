...
**LFGSpeaker** — LFG (Looking-For-Group) bulletin announcer for **WoW 2.4.3** (TBC, build 8606). Listens to chat events, parses recruitment bulletins, and **composes a spoken sentence at runtime** from short pre-rendered voice clips ("snippets"). Built for Russian-speaking TBC private servers (primary target: **Moonwell x5**) — Russian chat is handled by the sister addon RussianTranslator which translates Cyrillic to English before LFGSpeaker's filter sees it. LFGSpeaker itself only matches English text.

## Current state (v0.4.9, 2026-05-15)

**Published:** [github.com/angree/WOW243-LFG_Speaker](https://github.com/angree/WOW243-LFG_Speaker) (MIT). Working release with snippet-based playback, full config UI, minimap button. Voice: ElevenLabs Will (Relaxed Optimist, 1× multiplier), `eleven_multilingual_v2` model. 68 voice clips peak-normalized to -1 dB, OGG Vorbis mono 22050 Hz 64 kbps, ~1 MB total.

**Detection covers:** 25 TBC instances (16 5-mans + 9 raids), heroic/normal difficulty, role counts (tank/heal/dps/mdps/rdps + "many DPS" for 5+), group fill `N/M`, class-requested boolean.

**Filters:** per-instance N/H toggles, multi-role selection (dual-spec ready), 4 channel groups (Channels/Guild/Group/Local), per-sound + per-sender cooldowns, anti-LFG patterns for trade/guild ads/solo offers/bug reports/LFG-as-searcher.

**Snippet vocabulary** (lives in `Snippets.lua`, generated by `assemble_addon_sounds.py`): `lfg`, 25 instance names, `diff_heroic`/`diff_normal`, role+count clips, numbers 1-15, `out of`, `twenty five`/`forty`, `mod_class_req`, etc. Composed at runtime by `composeSentence()` in Core.lua, played sequentially via `OnUpdate` scheduler with configurable inter-clip gap.
...
```lua
PlaySoundFile("Interface\\AddOns\\LFGSpeaker\\sounds\\kara_tank.ogg")
-- or
PlaySoundFile([[Interface\AddOns\LFGSpeaker\sounds\kara_tank.ogg]])
```
...
## Architecture (actual, v0.4.9)

```
LFGSpeaker/
├── LFGSpeaker.toc          ## Interface: 20400, no BOM; lists files in load order
├── Patterns.lua            instance/role/marker tables + parseMessage()
├── Snippets.lua            AUTO-GENERATED ns.SNIPPETS = { key = {f=..., d=...} }
├── Sounds.lua              INSTANCE_GROUPS, RAID_KEYS, INSTANCE_COOLDOWN
├── Minimap.lua             draggable minimap button (no LibDBIcon)
├── Config.lua              programmatic config panel (no XML)
├── Core.lua                pipeline: filter → parse → compose → schedule playback
└── sounds/                 68× .ogg snippets (~1 MB total)
    ├── lfg.ogg
    ├── inst_kara.ogg ... (25 instance clips)
    ├── diff_heroic.ogg / diff_normal.ogg
    ├── role_tank_1.ogg, role_tank_2.ogg, role_heal_1.ogg, ...
    ├── num_1.ogg ... num_15.ogg
    ├── fill_out_of.ogg, fill_target_25.ogg, fill_target_40.ogg
    ├── conn_and.ogg, conn_needed.ogg, gen_all_roles.ogg
    └── mod_class_req.ogg, mod_fresh.ogg, mod_daily.ogg
```

**Sentence composition flow** (Core.lua `composeSentence(parsed)`):

```
[lfg] → [inst_<key>] → [diff_heroic|diff_normal]?
      → roles (all_roles_needed | enumerate with "and" + "needed")
      → group fill (num_N + out_of + num_M | fill_target_25 | fill_target_40)
      → [mod_class_req]?
```

Each snippet has a measured duration (`d`) from ffprobe at build time. The `OnUpdate` scheduler in Core.lua chains `PlaySoundFile` calls with `duration + gap` spacing — 2.4.3 has no playback-complete callback so we time-out manually.

**Build pipeline** (Python scripts in repo root):

- `generate_full_test.py` — generate MP3 variants via ElevenLabs API + build `test_output/preview.html` for picking voice variants
- `assemble_addon_sounds.py` — convert chosen MP3 variants → OGG with peak-normalization to -1 dB, write `LFGSpeaker/Snippets.lua` with durations
- `sync.bat` — `robocopy /MIR` LFGSpeaker/ to WoW AddOns folder

### Patterns.lua structure

```lua
ns.INSTANCES = {
    { key = "kara", display = "Karazhan",
      long  = { "karazhan" },        -- specific phrases matched literally
      short = { "kara" } },          -- 2-4 char abbrev, auto-bounded with wordBounded()
    { key = "bm",   display = "Black Morass", ... },
    -- ...
}

ns.ROLE_TOKENS = {
    mdps = { long = { "melee dps", "mili dps" }, short = { "mdps", "mdd" } },
    tank = { long = { "tanks", "tanker" },       short = { "tank" } },
    -- ...
}

ns.LFG_MARKERS_LONG / _SHORT  -- positive recruiter markers: lfm, lf1m..lf5m, lf, "looking for", "need.-tank"
ns.ANTI_LFG_LONG / _SHORT     -- wts/wtb/bug/guild/lfg + solo-offer phrases + guild-ad detection
```

Cyrillic patterns dropped in favour of relying on RussianTranslator. LFGSpeaker matches English only. Match logic: longest-alias-wins across all instances (`detectInstance` scans every instance and keeps the longest match — prevents `SP` falsely beating `Gruul` when both appear).
...
OUT_DIR = pathlib.Path("LFGSpeaker/sounds")
...
Run once per dictionary expansion. Commit the resulting `.ogg` files to the repo (under `LFGSpeaker/sounds/`). Distribution: clone or download release ZIP; `.ogg` files travel with the addon.
...
1. **Edit** Lua/sound files in `I:\GITHUB\WOW_LFG_Reader\LFGSpeaker\`.
2. **`sync.bat`** → `robocopy /MIR` to `C:\Gry\World of WarcraftOLD\Interface\AddOns\LFGSpeaker\`.
...
1. Edit `LFGSpeaker/LFGSpeaker.toc` `## Version: X.Y.Z`.
...
6. `powershell Compress-Archive LFGSpeaker → releases/wow243-lfgreader-vX.Y.Z.zip`.
